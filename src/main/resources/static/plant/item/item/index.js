...
$(function () {
    tab = $("#tab").dataTable({
        ...
        "render" : function (data, type, row, meta) {
            var innerStr = "";
            if(EDIT_QX) {
                innerStr += "<a title='编辑' href='javascript:;' onclick='edit(\"" + row.ITEM_ID + "\")' class='ml-5' style='text-decoration:none'><i class='Hui-iconfont'>&#xe6df;</i></a>";
            }
            if(DEL_QX) {
                innerStr += "<a title='删除' href='javascript:;' onclick='del(\"" + row.ITEM_ID + "\")' class='ml-5' style='text-decoration:none'><i class='Hui-iconfont'>&#xe6e2;</i></a>";
            }
            return innerStr;
            }
        }
        ...
        "sAjaxSource" : ctxPath + "/admin/item/api/getitems?now=" + new Date().getTime(),
        //服务器端，数据回调处理
...
//新增商品
var add = function () {
    showWindow("新增商品", ctxPath + "/admin/item/add");
};
...
var batchdel = function () {
    ...
    layer.confirm("确定要批量删除？", {
        ...
        sendRequest(ctxPath + "/admin/item/api/batchremoves", {batchremoves : batchremoves}, "POST", successFn);
    });
};
...
var onTableQuery = function () {
    ...
    params.ISSALE = $('#ISSALE').val();
    params.ITEMTYPE = $('#ITEMTYPE').val();
    params.ITEMPROPERTY = $('#ITEMPROPERTY').val();
    tableQuery(tab, params);
};

//编辑商品
var edit = function (id) {
    showWindow("编辑商品", ctxPath + "/admin/item/edit/" + id);
};
...
var del = function (id) {
    layer.confirm("确定要删除？", {
        ...
        layer.close(index);
        sendRequest(ctxPath + "/admin/item/api/batchremoves", {batchremoves : id}, "POST", successFn);
    });
};