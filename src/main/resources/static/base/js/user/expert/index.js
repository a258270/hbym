...
$(function () {
    tab = $("#tab").dataTable({
        ...
        "sAjaxSource" : ctxPath + "/admin/expert/api/getexperts?now=" + new Date().getTime(),
        //服务器端，数据回调处理
...
//新增用户
var add = function () {
    showWindow("新增用户", ctxPath + "/admin/expert/add");
};
...
var datadel = function () {
    ...
    layer.confirm("确定要冻结这些用户？", {
        ...
        sendRequest(ctxPath + "/admin/expert/api/batchremoves", {batchremoves : batchremoves}, "POST", successFn);
    });
...
var user_edit = function (id) {
    showWindow("编辑用户", ctxPath + "/admin/expert/edit/" + id);
};
...
var user_del = function (id) {
    layer.confirm("确定要冻结该用户？", {
        ...
        layer.close(index);
        sendRequest(ctxPath + "/admin/expert/api/batchremoves", {batchremoves : id}, "POST", successFn);
    });
};
...
var user_undel = function (id) {
    layer.confirm("确定要解冻该用户？", {
        ...
        layer.close(index);
        sendRequest(ctxPath + "/admin/expert/api/unremove", {USER_ID : id}, "POST", successFn);
    });
};