...
<script language="JavaScript">
    ...
    KindEditor.ready(function(K) {
        ...
        prettyPrint();
        KE.text("CONTENT", editorCONTENTBK);
    });
    $(function () {
        $("input[type=file]").change(function(){$(this).parents(".sp-uploader").find(".sp-filename").val($(this).val());});
        $("input[type=file]").each(function(){
            if($(this).val()==""){$(this).parents(".sp-uploader").find(".sp-filename").val("不更换封面无需选择图片");}
        });
        $("#form").validate({
            rules:{
                TITLE:{
...
<article class="page-container">
    <form action="${ctxPath}/plant/teacher/api/edit_article/${article.ARTICLE_ID}" method="post" enctype="multipart/form-data" class="form form-horizontal" id="form">
        <div class="row cl">
            <label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>标题：</label>
            <div class="formControls col-xs-8 col-sm-9">
                <input type="text" class="input-text" value="<#if article?? && article.TITLE??>${article.TITLE}</#if>" placeholder="请输入标题" id="TITLE" name="TITLE">
            </div>
        </div>
        <div class="row cl">
            <label class="form-label col-xs-4 col-sm-3">副标题：</label>
            <div class="formControls col-xs-8 col-sm-9">
                <input type="text" class="input-text" value="<#if article?? && article.SUBTITLE??>${article.SUBTITLE}</#if>" placeholder="请输入副标题" id="SUBTITLE" name="SUBTITLE">
            </div>
        </div>
        <div class="row cl">
        ...
        <div class="row cl">
            ...
            <div class="formControls col-xs-8 col-sm-9">
                <textarea id="ABSTRACT" name="ABSTRACT" class="textarea" cols="100" rows="8"><#if article.ABSTRACT??>${article.ABSTRACT}</#if></textarea>
            </div>
        </div>
        <div class="row cl">
            <label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>内容：</label>
            <div class="formControls col-xs-8 col-sm-9">
                <textarea id="CONTENT" name="CONTENT" class="textarea" cols="100" rows="8"><#if article?? && article.CONTENT??>${article.CONTENT}</#if></textarea>
            </div>
        </div>
        <div class="row cl">
            <label class="form-label col-xs-4 col-sm-3">排序序号：</label>
            <div class="formControls col-xs-8 col-sm-9">
                <input type="text" class="input-text" value="<#if article?? && article.MSORT??>${article.MSORT?c}</#if>" placeholder="请输入排序序号" id="MSORT" name="MSORT">
            </div>
        </div>
        <div class="row cl">
