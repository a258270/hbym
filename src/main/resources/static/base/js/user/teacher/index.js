...
$(function () {
    tab = $("#tab").dataTable({
        ...
        "sAjaxSource" : ctxPath + "/admin/teacher/api/getteachers?now=" + new Date().getTime(),
        //服务器端，数据回调处理
...
//新增用户
var add = function () {
    showWindow("新增用户", ctxPath + "/admin/teacher/add");
};
...
var datadel = function () {
    ...
    layer.confirm("确定要冻结这些用户？", {
        ...
        sendRequest(ctxPath + "/admin/teacher/api/batchremoves", {batchremoves : batchremoves}, "POST", successFn);
    });
...
var user_edit = function (id) {
    showWindow("编辑用户", ctxPath + "/admin/teacher/edit/" + id);
};
...
var user_del = function (id) {
    layer.confirm("确定要冻结该用户？", {
        ...
        layer.close(index);
        sendRequest(ctxPath + "/admin/teacher/api/batchremoves", {batchremoves : id}, "POST", successFn);
    });
};
...
var user_undel = function (id) {
    layer.confirm("确定要解冻该用户？", {
        ...
        layer.close(index);
        sendRequest(ctxPath + "/admin/teacher/api/unremove", {USER_ID : id}, "POST", successFn);
    });
};