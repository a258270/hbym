package com.cms4j.plant.news.file.controller;
...
public class KindFileUploadController extends ApiBaseController {
    ...
    @RequestMapping(value = "/plant/{fileType}/download/{fileName}.{suffixName}")
    public void downloadFile(HttpServletRequest request, HttpServletResponse response, @PathVariable String fileType, @PathVariable String fileName, @PathVariable String suffixName) {
        InputStream is = null;
        OutputStream os = null;
        try {
            String path = baseSetting.getUpload_path() + "/" + fileName + "." + suffixName;
            if("file".equals(fileType)){
                path = baseSetting.getUpload_path() + "/" + fileName + "." + suffixName;
            }

            if("schead".equals(fileType)){
                path = baseSetting.getSc_head() + "/" + fileName + "." + suffixName;
            }

            if("scvideo".equals(fileType)) {
                path = baseSetting.getScvideo_path() + "/" + fileName + "." + suffixName;
            }

            if("scimage".equals(fileType)) {
                path = baseSetting.getScimage_path() + "/" + fileName + "." + suffixName;
            }
    ...
    public Map<String, Object> upload(HttpServletRequest request, HttpServletResponse response) {
        ...
        if(curUser == null)
            curUser = SessionUtil.getCurUser();
        //创建文件夹
